  ...
  const celestialProgress = (() => {
    if (isDay) {
      ...
      if (currentTime >= sunset) {
        ...
        const nextSunrise = SunCalc.getTimes(
          nextDay,
          LAT,
          LNG,
        ).sunrise.getTime();
        ...
        return Math.max(
          0,
          Math.min(1, (currentTime - prevSunset) / nightDuration),
        );
      }
    }
  })();
    ...
    <g id={"heaven"}>
      <defs>
        <linearGradient id="dayskyGradient" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" style={{ stopColor: "#0376BF" }} />
          <stop offset="10%" style={{ stopColor: "#0376BF" }} />
          <stop offset="60%" style={{ stopColor: "#A8D0F5" }} />

          <stop offset="100%" style={{ stopColor: "#A8D0F5" }} />
        </linearGradient>
      ...
      {isDay && <Sun x={sunPosition.x} y={sunPosition.y} />}

      {!isDay && <Moon x={moonPosition.x} y={moonPosition.y} />}
    </g>
  );
};
